...
function DraggableLinkItem({ link, onRemove }: { link: Link; onRemove: (id: string) => void }) {
  ...
  return (
    ...
    >
      ...
      {/* ── 노드 정보 연결 (항상 표시) ── */}
      <div className="border-t border-border/60 bg-slate-50/60 px-4 py-2">
        <div className="flex items-center gap-2">
          ...
          <TooltipProvider delayDuration={200}>
            <Tooltip>
              <TooltipTrigger asChild>
                <span className="flex items-center gap-1 text-xs font-semibold text-muted-foreground w-[130px] shrink-0 cursor-default whitespace-nowrap">
                  노드 정보 연결
          ...
          {/* 소스 측 */}
          <span className="inline-flex items-center rounded px-2 h-7 bg-white border border-border text-[11px] font-medium text-slate-600 shrink-0">{link.sourceColumn || "—"}</span>
          ...
          <Select defaultValue={link.sourceTable}>
            <SelectTrigger className="bg-white h-7 text-xs w-[130px]"><SelectValue placeholder="시작 시트명"/></SelectTrigger>
            <SelectContent>{TABLE_OPTIONS.map(o=><SelectItem key={o} value={o}>{o}</SelectItem>)}</SelectContent>
          </Select>
          <Select defaultValue={link.sourceNodeKey||"none"} disabled>
            <SelectTrigger className="h-7 text-xs w-[80px] opacity-40 cursor-not-allowed"><SelectValue/></SelectTrigger>
          ...
          {/* 타깃 측 */}
          <span className="inline-flex items-center rounded px-2 h-7 bg-white border border-border text-[11px] font-medium text-slate-600 shrink-0">{link.targetColumn || "—"}</span>
          <div className="flex items-center shrink-0"><div className="w-3 h-px bg-muted-foreground/30"/><ArrowRight className="w-3 h-3 text-muted-foreground/30 -ml-px"/></div>
          <Select defaultValue={link.sourceTable}>
            <SelectTrigger className="bg-white h-7 text-xs w-[130px]"><SelectValue placeholder="도착 시트명"/></SelectTrigger>
            <SelectContent>{TABLE_OPTIONS.map(o=><SelectItem key={o} value={o}>{o}</SelectItem>)}</SelectContent>
          </Select>
