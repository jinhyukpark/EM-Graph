...
export default function ProjectView() {
  const [nodes, setNodes, onNodesChange] = useNodesState(createRadialLayout());
  const [edges, setEdges, onEdgesChange] = useEdgesState([]);
  const [activeTab, setActiveTab] = useState("nodes");
  const [selectedNode, setSelectedNode] = useState<any>(null);
  const [menu, setMenu] = useState<{ id: string | null; top?: number; left?: number; right?: number; bottom?: number } | null>(null);
  ...
  const handleSidebarNodeSelect = (nodeId: string) => {
      ...
      sidebarControls={
        <div className="flex bg-secondary/20 p-1 rounded-lg border border-border/50">
          <Button
            ...
            size="sm"
            className={cn("flex-1 h-7 text-xs font-medium", activeTab === "nodes" && "bg-background shadow-sm text-primary")}
            onClick={() => setActiveTab("nodes")}
          >
            <Database className="w-3.5 h-3.5 mr-1.5" /> Graph Connect
          </Button>
          <Button
            variant={activeTab === "nav" ? "secondary" : "ghost"}
            size="sm"
            className={cn("flex-1 h-7 text-xs font-medium", activeTab === "nav" && "bg-background shadow-sm text-primary")}
            onClick={() => setActiveTab("nav")}
          >
            <List className="w-3.5 h-3.5 mr-1.5" /> Node List
          </Button>
        </div>
      }
    >
      <div className="w-full h-full relative" ref={ref}>
        ...
        >
          ...
          {menu && (
            <div
              className="absolute z-50 min-w-[180px] bg-background border border-border rounded-lg shadow-xl p-1.5 animate-in fade-in zoom-in-95 duration-100"
              style={{
                top: menu.top,
                left: menu.left,
                right: menu.right,
            ...
            >
              <div className="flex flex-col gap-0.5">
                <Button variant="ghost" size="sm" className="justify-start h-8 px-2 text-xs font-medium" onClick={() => {
                  setNodes([...nodes]); // Refresh simulation
                  setMenu(null);
                }}>
                ...
                <Button variant="ghost" size="sm" className="justify-start h-8 px-2 text-xs font-medium text-primary hover:text-primary hover:bg-primary/10" onClick={() => {
                   // Add random node logic here
