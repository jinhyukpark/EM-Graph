...
function DraggableLinkItem({ link, onRemove }: { link: Link; onRemove: (id: string) => void }) {
  ...
  return (
    <Reorder.Item
      ...
      dragListener={false}
      dragControls={dragControls}
      className="flex items-center p-4 rounded-lg border border-border bg-white shadow-sm"
      whileDrag={{ scale: 1.02, boxShadow: "0 8px 25px rgba(0,0,0,0.12)", zIndex: 50 }}
      transition={{ duration: 0.2 }}
    >
      <div
...
export default function GraphBuilder() {
  ...
  const addLink = () =>
    setLinks([...links, { id: generateId(), sourceTable: "", sourceColumn: "", targetTable: "", targetColumn: "" }]);
  const removeLink = (id: string) => setLinks(removeItemById(links, id));
  const addNode = () =>
    setNodes([...nodes, { id: generateId(), table: "", labelField: "", sizeField: "", colorField: "", icon: "Circle" }]);
  const removeNode = (id: string) => setNodes(removeItemById(nodes, id));
        ...
        <div className="flex-1 flex flex-col max-w-5xl mx-auto w-full p-8 overflow-y-auto">
          <div className="mb-8">
            <div className="flex items-center gap-3 mb-2">
              ...
              <h1 className="text-2xl font-bold" data-testid="text-graph-builder-title">Graph Builder</h1>
            </div>
          ...
          <div className="space-y-6">
            <Card className="border-border shadow-sm">
              <CardHeader className="pb-3 border-b border-border/50 bg-secondary/5">
                <div className="flex items-center justify-between">
                  ...
                  <Button size="sm" variant="outline" className="h-8 gap-1" onClick={addNode} data-testid="button-add-node">
                    <Plus className="w-3 h-3" /> Add Node
                  </Button>
                </div>
              </CardHeader>
              <CardContent className="p-6">
                <Reorder.Group axis="y" values={nodes} onReorder={setNodes} className="grid gap-6">
                  {nodes.map((node) => (
            ...
            <Card className="border-border shadow-sm overflow-hidden">
              <div className="p-4 border-b border-border bg-white flex items-center justify-between">
                ...
                <Button size="sm" onClick={addLink} className="gap-1 bg-blue-600 hover:bg-blue-700 text-white" data-testid="button-add-link">
            ...
            <div className="flex justify-end gap-3 pt-4">
              <Button variant="outline" size="lg" data-testid="button-reset">Reset</Button>
              <Button size="lg" className="bg-black hover:bg-black/90 text-white px-8" data-testid="button-generate-graph">
                Generate Graph
              </Button>
            </div>
          </div>
